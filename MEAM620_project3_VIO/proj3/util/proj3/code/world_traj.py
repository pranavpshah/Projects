...
class WorldTraj(object):
    ...
    def __init__(self, world, start, goal):
        ...
        self.waypoints = np.concatenate((self.waypoints[0:-1:6], self.path[-1].reshape((1,3))), axis = 0)
        # self.waypoints = np.concatenate((self.waypoints[0:-1:3], self.path[-1].reshape((1,3))), axis = 0)
        
        rows = self.waypoints.shape[0]
        #self.waypoints = np.delete(self.waypoints, rows-2, 0)
        ...
        self.avg_speed = 2.8 #defining average speed for each segment in m/s
        # self.avg_speed = 1
        self.duration = self.distances/self.avg_speed #duration of each segment computed considering average speed of 3.6 m/s
        # self.duration = (self.distances**(1/3))/2.75

        for i in range(1,len(self.vectors) - 1):
            vec1 = self.vectors[i]
            vec2 = self.vectors[i+1]
            theta = np.arccos(np.sum(vec1*vec2))
            if(theta > 0.4):
                self.duration[i] = self.duration[i]*1.5*np.sin(theta)
            else:
        ...
        # # self.duration[size//2 - 1] = self.duration[size//2 - 1]*2
        # # self.duration[size//2] = self.duration[size//2]*2
        # # self.duration[size//2 + 1] = self.duration[size//2 + 1]*2
        # self.duration[-4] = self.duration[-4]*2
        # self.duration[-3] = self.duration[-3]*3
        # self.duration[-2] = self.duration[-2]*2
